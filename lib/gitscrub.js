...
gitscrub.selectRepos = function(reposToSelect, logValue) {
  if(Array.isArray(reposToSelect)) {
    gitscrub.getAllRepos(function(data) {
      // first retrieve existing object from repos_to_scrub.json
      fs.readFile(path.join(__dirname, 'repos_to_scrub.json'), 'utf8', function(err, data) {
        if(err) {
          throw err
        }
        ...
        fs.writeFile(path.join(__dirname, 'repos_to_scrub.json'), JSON.stringify(reposToScrub, null, 2), function(err) {
          if(err) {
            throw err
          }
          logValue(true)
    ...
    console.log("Argument must be an array of repos to scrub")
    logValue(false)
  }
}

gitscrub.getSelRepos = function(logValue) {
  fs.readFile(path.join(__dirname, 'repos_to_scrub.json'), 'utf8', function(err, data) {
    if(err) {
      throw err;
    }
...
gitscrub.parseReadMe = function(readme, options, callback) {
  ...
  readMeParsed.each(function(line) {
    if (line.substring(0, 2) == "# ") {
      ...
      formattedReadMe[header] = ""
    } else if (header != "") {
      formattedReadMe[header] += line
    }
  })
